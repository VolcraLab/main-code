...
inline void runCustomPose() {
  Serial.println(F("CUSTOM"));
  setFaceWithMode("cute", FACE_ANIM_LOOP); // ganti face sesuai selera

  // ── Frame 1 ──
  setServoAngle(R1, 135); setServoAngle(L2, 135); setServoAngle(R2, 45);  setServoAngle(L1, 45);
  setServoAngle(R3, 180); setServoAngle(R4, 0);   setServoAngle(L3, 0);   setServoAngle(L4, 180);
  delayWithFace(400);  // ← PENTING: ganti delay() → delayWithFace()

  // ── Frame 3 ──
  setServoAngle(R3, 90); setServoAngle(L4, 90);
  delayWithFace(400);
  ...
  // ── Frame 10 ──
  setServoAngle(R4, 90); setServoAngle(L3, 90); setServoAngle(L4, 90);
  delayWithFace(400);
  ...
  runStandPose(1);
  if (currentCommand == "custom") currentCommand = "";
}
